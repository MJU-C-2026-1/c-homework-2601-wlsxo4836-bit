[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/1R97bbXI)
# 💻 [2026-01] 나만의 C언어 소프트웨어 개발 프로젝트

## 1. 시나리오 제목
* 환전 및 해외 직구 계산기

<br>


## 2. 시나리오 (5~10줄)
* 이 프로그램은 해외에서 판매하는 상품의 가격과 그 외화의 현재 환율을 입력받아 한화 최종 금액이 얼마인지 나타내는 환전 및 해외 직구 계산기입니다. 사용자는 먼저 해외에서 판매하는 상품의 가격과 그 외화의 환율을 입력합니다. 시스템은 이를 바탕으로 환율을 적용하여 상품의 가격을 계산하고, 해외 결제 수수료, 국제 배송비를 포함하여 총 한화 금액이 얼마인지 계산하여 나타냅니다. 최종적으로는 관세 적용, 외화의 종류, 장바구니 등을 만들어 계산기의 확장을 이끌어내 시스템을 완성할 계획입니다.
<br>


## 3. 예상 기능 및 메뉴 (최소 5개)
1. 상품 이름의 첫 글자
2. 상품 가격 (달러)
3. 현재 환율 (1달러당 원화)
4. 해외 결제 수수료 (2%)
5. 국제 배송비 (원)
6. 최종 결제 금액 (원)
7. 금액에 따라 관세 적용


<br>

---

# 🚀 [버전별 개발 일지 & AI 협업 기록]

## 🟦 [1차 과제: V1.0] 기초 뼈대 구축 (입출력과 변수)
### **✨V1.0 개발 내용:**
  * 사용자에게 상품의 첫 글자, 상품의 가격, 현재 환율을 입력받아 상품의 총 한화 가격을 계산하여 출력합니다.
  * 상품의 첫 글자, 상품 가격, 현재 환율을 입력받는 화면을 구현했습니다.
  * 사칙연산을 활용해 해외 결제 수수료, 국제 배송비를 포함해 '총 한화 가격'을 계산하여 출력했습니다.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1**
    * **프롬프트 요약:** "C언어 4단계 과제 로드맵(입출력 ➔ 조건문 ➔ 반복문 ➔ 배열/함수)에 적합한 환전 및 해외 직구 계산기 및 단계별 기능 확장 로드맵 기획 요청"
    * **적용 내용:** 단순한 해외 직구 계산기를 넘어, '환율 입력(1차) ➔ 관세 등록(2차) ➔ 여러 가지 외화(3차) ➔ 장바구니(4차)'로 이어지는 논리적인 시스템 확장 로드맵과 예상 메뉴를 확립함.
      
 2. **내용 2**
    * **프롬프트 요약:** "1차 과제 필수 요건(변수 5개, 자료형 3개 이상)을 충족하기 위한  환전 및 해외 직구 계산기에 필요한 함수 종류 및 데이터 구조 모델링 논의"
    * **적용 내용:** 단순히 환율만을 계산하는 게 아닌, 데이터 특성에 맞춰 문자형(`char` - 상품의 첫 글자), 정수형(`int` - 상품 가격, 국제 배송비), 실수형(`double` - 현재 환율, 해외 결제 수수료, 최종 결제 금액)으로 자료형을 명확히 분리하여 함수의 종류와 입력 데이터의 구조적 완성도를 높임.
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** 실행할 때 한글이 깨지는 현상이 발생하여 수업 때 배운 stdlib 전처리기로 해결하려 하였으나 코드가 기억이 안 남
     * **해결:** AI에게 질문하여 stdlib 전처리기의 여러 가지 코드를 입력받음
  2. **문제 2:** 실행 화면에 나오는 Active code page: 65001 메시지가 지저분하여 지우려고 함
     * **해결:** AI 검색을 통해 system 함수 안에 들어가는 명령어 뒤에 **> nul**을 붙여 메시지가 뜨지 않게 해결함

### **📁 증빙 자료:**
  * [AI협업증명(1차과제).pdf](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/3457ef73b7a035992f5b498c0e576ba2d3eac83b/AI%ED%98%91%EC%97%85%EC%A6%9D%EB%AA%85(1%EC%B0%A8%EA%B3%BC%EC%A0%9C).pdf)
  * [AI협업증명2(1차과제).pdf](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/cc1798df72af478f30f5969ba6362a64f97993dc/AI%ED%98%91%EC%97%85%EC%A6%9D%EB%AA%852(1%EC%B0%A8%EA%B3%BC%EC%A0%9C).pdf)
  * [1차과제_실행결과.jpg](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/753895fe075745037b1a0f1dcfd531d8fc640343/1%EC%B0%A8%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC.jpg)

<br>

## 🟩 [2차 과제: V2.0] 조건 분기 적용 (조건문)
### **✨2차 과제 업데이트 내용:**
  * 조건문을 이용하여 물건에 관세를 추가하는 과정을 거쳤습니다.
  * 상품의 가격이 150$이상일 경우 상품 금액의 10%을 추가하는 코드를 추가하여 가격을 책정하여 출력했습니다.
  * 상품이 관세가 적용되었을 시 안내하는 문장을 출력했습니다.
    
### **🤖 AI 파트너십 과정**
 1. **내용 1**
    * **프롬프트 요약:** "기존 V 1.0의 과제에 어느 가격 이상일 때 관세를 붙일 수 있게 조건문인 if문을 활용할 때 고려해야 할 사항들 요청"
    * **적용 내용:** 관세를 정하기 위한 변수 선언, 관세는 언제 계산되는지에 대한 순서, 관세 비율 등을 정함.
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** 화면에 출력하는 코드에 %를 나타내고 싶은데 화면에 뜨지 않아 이유와 해결법을 찾음
     * **해결:** %은 변수 표현을 위해 하나로 작성하면 안되고 %%로 표시해야 함을 알아 해결함
     
### **📁 증빙 자료:**
  * [2차 과제 실행결과 1.png](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/main/2%EC%B0%A8%20%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC%201.png)
  * [2차 과제 실행결과 2.png](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/33b9d8562f0f0b55257394ef8371515e7803188c/2%EC%B0%A8%20%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC%202.png)
  * [AI협업증명(2차과제).pdf](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/33b9d8562f0f0b55257394ef8371515e7803188c/AI%ED%98%91%EC%97%85%EC%A6%9D%EB%AA%85(2%EC%B0%A8%20%EA%B3%BC%EC%A0%9C).pdf)
  * [AI협업증명2(2차과제).pdf](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/615a58ec58aed387b98afc053d6b5bdb420c2a36/AI%ED%98%91%EC%97%85%EC%A6%9D%EB%AA%852(2%EC%B0%A8%EA%B3%BC%EC%A0%9C).pdf)
<br>

## 🟨 [3차 과제: V3.0] 무한 루프와 메뉴 시스템 (반복문)
### **✨3차 과제 업데이트 내용:**
  * 함수를 이용하여 main함수로 모든 작업을 하던 기존의 코드를 수정했습니다.
  * 상품의 누적 금액을 기록하여 확인할 수 있는 변수 및 코드를 추가했습니다.
  * while(1)코드로 무한루프를 이용하여 직접 종료하기 전까지 종료되지 않는 코드를 추가했습니다.
    
### **🤖 AI 파트너십 과정**
 없음
    
### **🛠️ Troubleshooting & 기술 회고:**
  1. **문제 1:** 처음에 입력할 때 문자를 입력하게 되면 잘못된 숫자라고 뜨나 다시 입력하지 못하고 무한반복됨
     * **해결:** 문자는 입력 버퍼를 남기게 되어 무한루프되며 이 버퍼를 지워주어야 다시 제대로 실행됨을 알아내고 while (getchar() != '\n'); 코드를 이용하여 해결함
     
### **📁 증빙 자료:**
  * [3차_AI협업캡처.pdf](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/7bcbd2e7b0a89a5b3d557493cb960f6c8e4424b7/AI%ED%98%91%EC%97%85%EC%A6%9D%EB%AA%85(3%EC%B0%A8%EA%B3%BC%EC%A0%9C).pdf)
  * [3차과제_실행결과1.jpg](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/7bcbd2e7b0a89a5b3d557493cb960f6c8e4424b7/3%EC%B0%A8%20%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC%201.png)
  * [3차과제_실행결과2.jpg](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/7bcbd2e7b0a89a5b3d557493cb960f6c8e4424b7/3%EC%B0%A8%20%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC%202.png)
<br>

### 🟥 [4차 과제: V4.0] 모듈화 및 데이터 확장 (배열과 함수) - 🌟최종 완성 -- 향후 작성 예정
### **✨4차 과제 업데이트 내용:**
  * 배열을 추가하여 여러 상품을 등록할 수 있도록 코드를 수정했습니다.
  * 포인터 변수를 추가하여 코드를 단순화하고 배열을 사용하기 쉽도록 수정했습니다.
    
### **🤖 AI 파트너십 과정**
    없음 
    
### **🛠️ Troubleshooting & 기술 회고:**
    없음
     
### **📁 증빙 자료:**
  * [4차과제_실행결과1.jpg](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/63dd158b4b57e8ba396d24ed1f00b727bb5b79a2/4%EC%B0%A8%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC%201.png)
  * [4차과제 실행결과2.jpg](https://github.com/MJU-C-2026-1/c-homework-2601-wlsxo4836-bit/blob/63dd158b4b57e8ba396d24ed1f00b727bb5b79a2/4%EC%B0%A8%EA%B3%BC%EC%A0%9C%20%EC%8B%A4%ED%96%89%EA%B2%B0%EA%B3%BC%202.png)
  * 
<br>
